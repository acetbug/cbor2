...
import type { DecodeStreamOptions, DecodeValue, MtAiValue, Sliceable } from './options';
import {
  MT, NUMBYTES, SYMS,
} from './constants';
import { base64ToBytes, hexToU8, subarrayRanges } from './utils';
import { Simple } from './simple';
import { parseHalf } from './float';

const TD = util.TextDecoder.create('utf8', { fatal: true, ignoreBOM: true });
    ...
    switch (mt) {
      ...
      case MT.BYTE_STRING:
        if (val === Infinity) {
          ...
          yield [mt, ai, this.#read(val as number), prevOffset, val as number];
        }
        break;
      case MT.UTF8_STRING:
        if (val === Infinity) {
      ...
      case MT.SIMPLE_FLOAT: {
        ...
        yield [mt, ai, val, prevOffset, oval as number];
        break;
      }
    }
  }
