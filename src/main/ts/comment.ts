...
import type { CommentOptions, MtAiValue, Parent, RequiredCommentOptions, RequiredDecodeOptions, } from './options';
import { MT, NUMBYTES, SYMS } from './constants';
import { getEncoded, type OriginalEncoding, saveEncoded } from './box';
import { getRanges, subarrayRanges, u8toHex } from './utils';
      ...
      if (ranges?.length) {
        ...
        for (const [start, len, type] of ranges) {
          ...
          if (type === '<<') {
            ...
            ret += ind;
            ret += "'";
            ret += TD.decodeToString(enc.subarray(start, start + len));
            ret += "'\n";
          }
          if (start > numLen) {
